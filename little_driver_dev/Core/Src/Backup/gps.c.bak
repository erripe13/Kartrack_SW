...
void L76_PrintExample(void) {
    ...
    if (data.fix_quality == 0) {
    	printf("Pas de fix GPS %d satellites visibles.\r\n", data.satellites);
    } else {
        printf("GPS Fix: Lat=%.5f, Lon=%.5f, Alt=%.1f m\r\n",
               data.latitude, data.longitude, data.altitude);
        printf("UTC Time=%02d:%02d:%05.2f, Date=%02d/%02d/%04d, Speed=%.1f knots\r\n",
               data.hours, data.minutes, data.seconds,
               data.day, data.month, data.year, data.speed);
    }
}
